class Solution {
  ...
  #dfs(grid, r, c, steps = 0, visited = new Set()) {
      const coord = `${r}-${c}`;
      if (r < 0 || r >= this.m || c < 0 || c >= this.n || visited.has(coord) || this.#isWater(grid[r][c])) {
          return Infinity;
      }
      if (this.#isTreasure(grid[r][c])) return steps;
      ...
      return Math.min(
        this.#dfs(grid, r - 1, c, steps + 1, visited),
        this.#dfs(grid, r, c + 1, steps + 1, visited),
        this.#dfs(grid, r + 1, c, steps + 1, visited),
        this.#dfs(grid, r, c - 1, steps + 1, visited),
      );
  }
