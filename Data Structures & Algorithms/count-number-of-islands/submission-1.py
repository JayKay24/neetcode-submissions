                ...
                if grid[i][j] == "1":
                    ...
                    for x, y in directions:
                        ...
                        if coord2.is_in_bounds(m, n) and grid[r][c] == "1":
                            disjoint_set.union(coord1.get_str_coord(), coord2.get_str_coord())
...
class Disjoint_Set:
    ...
    def add(self, coord: Coord) -> None:
        dict_key = coord.get_str_coord()
        if dict_key not in self._elements:
            self._elements[dict_key] = Info(dict_key)
            self.count += 1

    def find(self, elem: str) -> str | None:
